...
app.post( '/login', async ( req, res ) => {
    ...
    try {
        ...
        if ( !email || !password ) return res.status(400).json({error: 'User data missing!'})
        ...
        if ( check ) {
            // Set Token
            const accessToken = jwt.sign( user.username, process.env.ACCESS_SECRET )
            
            res.json({message: 'Login successful!', accessToken, username: user.username})
        } else {
...
app.get( '/foods', authenticateToken, async ( req, res ) => {
    ...
    if (email !== null  || email !== undefined || email !== '') {
        const food = await Food.findOne( { email } )
        
        if ( food == null ) return res.json( {message: 'User authentication successful!', result: null} )
        
        return res.json( {message: 'User authentication successful!', result: food} )
    }
    Food.find()
        .then(
            result => res.json( {message: 'User authentication successful!', result} ) )
...
app.post( '/foods', authenticateToken, ( req, res ) => {
    try {
        ...
        // Pushing to Database
        const newFood = new Food( { email: email.toLowerCase(), entries } )
        newFood.save().then( () => {
